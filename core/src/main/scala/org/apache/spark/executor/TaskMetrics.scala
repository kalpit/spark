...
class TaskMetrics extends Serializable {
  ...
  /**
   * Custom task-specific metrics. Any piece of Spark machinery that cares to track custom
   * task-related metrics can now use the setCustomMetric(name,value) method on TaskMetrics
   * to do that.
   * e.g. Any RDD that wants to track a custom metric of its interest (related to execution of
   * a task) can call setCustomMetric(name,value) inside its compute() method.
   * Map Key -> name of custom metric
   * Map Value -> list of numeric values for custom metric
   */
  private val _customMetrics: scala.collection.mutable.HashMap[String, List[Long]] =
